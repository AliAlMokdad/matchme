...
const CHOICE_GROUPS = {
  ...
  travel_freq: {
    groups: [['monthly', 'few_a_year'], ['few_a_year', 'once_a_year'], ['once_a_year', 'rarely']],
    crossPenalty: { 'monthly-rarely': 30 },
  },
  travel_style: {
    groups: [['backpacker', 'mid_range'], ['mid_range', 'luxury'], ['whatever', 'backpacker'], ['whatever', 'mid_range'], ['whatever', 'luxury']],
    crossPenalty: { 'backpacker-luxury': 25 },
  },
  travel_planning: {
    groups: [['full_itinerary', 'research_heavy'], ['loose_plan', 'spontaneous'], ['research_heavy', 'loose_plan']],
    crossPenalty: { 'full_itinerary-spontaneous': 30 },
  },
  travel_pace: {
    groups: [['slow_deep', 'balanced_pace'], ['balanced_pace', 'fast_packed']],
    crossPenalty: { 'slow_deep-fast_packed': 35 },
  },
  travel_type: {
    groups: [['cities', 'cultural'], ['nature', 'beaches'], ['mix', 'cities'], ['mix', 'nature'], ['mix', 'beaches'], ['mix', 'cultural']],
    crossPenalty: {},
  },
  travel_budget_split: {
    groups: [['split_equal', 'by_income'], ['take_turns', 'pool_together'], ['split_equal', 'take_turns']],
    crossPenalty: {},
  },
...
function personalityDimensions(r) {
  ...
  const lifestyle = avg([
    ...
    choiceScore('travel_freq', r.travel_freq, { monthly: 5, few_a_year: 4, once_a_year: 3, rarely: 2 }),
  ]) * 20;
...
const SECTION_QUESTIONS = {
  ...
  lifestyle:     ['work_life_balance', 'personal_growth', 'finances', 'travel_freq'],
  communication: ['discussion_style', 'conflict_resolution', 'love_language', 'express_feelings', 'constructive_feedback', 'communication_freq'],
  ...
  travel:        ['travel_style', 'travel_planning', 'travel_pace', 'travel_type', 'travel_budget_split', 'travel_solo_group', 'travel_comfort'],
};
...
const QUESTION_META = {
  ...
  work_life_balance: 'scale', personal_growth: 'scale', finances: 'choice', travel_freq: 'choice',
  ...
  travel_style: 'choice', travel_planning: 'choice', travel_pace: 'choice', travel_type: 'choice',
  travel_budget_split: 'choice', travel_solo_group: 'scale', travel_comfort: 'scale',
};
...
function calculateMatch(a, b) {
  ...
  // Travel score — weighted heavily on travel-specific questions
  const travel = weightedScore(sections, {
    travel: 0.70, lifestyle: 0.20, values: 0.10,
  });
  ...
  const lfa = ar.looking_for || [];
  const lfb = br.looking_for || [];

  const bothTraveling = lfa.includes('travel') && lfb.includes('travel');

  // Overall: average of applicable scores
  const applicableScores = [romantic, roommate, friendship];
  if (bothTraveling) applicableScores.push(travel);
  const overall = Math.round(applicableScores.reduce((s, v) => s + v, 0) / applicableScores.length);

  // Key open-ended answers to display side by side
  const OPEN_QS = [
    { id: 'commitment_meaning', label: 'What commitment means to me' },
    { id: 'ideal_partnership',  label: 'My ideal partnership' },
    { id: 'weekend_style',      label: 'Perfect weekend' },
    { id: 'how_you_unwind',     label: 'How I unwind' },
    { id: 'like_about_self',    label: 'What I like about myself' },
    { id: 'pet_peeves',         label: 'My pet peeves' },
    { id: 'life_goals',         label: 'Goals for the next 5 years' },
    { id: 'dream_destination',  label: 'Dream destination' },
    { id: 'worst_travel',       label: 'How I handle travel challenges' },
  ];
  const openAnswers = OPEN_QS
    .map(q => ({ question: q.label, a: ar[q.id] || null, b: br[q.id] || null }))
    .filter(q => q.a || q.b);
  ...
  return {
    ...
    romantic: {
      score: romantic,
      bothSelected: lfa.includes('romantic') && lfb.includes('romantic'),
      ...generateCategoryNarrative(a, b, ar, br, 'romantic', sections),
    },
    roommate: {
      score: roommate,
      bothSelected: lfa.includes('roommate') && lfb.includes('roommate'),
      ...generateCategoryNarrative(a, b, ar, br, 'roommate', sections),
    },
    friendship: {
      score: friendship,
      bothSelected: lfa.includes('friend') && lfb.includes('friend'),
      ...generateCategoryNarrative(a, b, ar, br, 'friendship', sections),
    },
    travel: {
      score: travel,
      bothSelected: bothTraveling,
      ...generateCategoryNarrative(a, b, ar, br, 'travel', sections),
    },
    overall,
    profiles: {
...
function generateCategoryNarrative(a, b, ar, br, type, sections) {
  ...
  if (type === 'travel') {
    if (sections.travel >= 80) reasons.push(`Highly compatible travel DNA — you naturally align on style, pace, and planning`);
    if (sections.travel >= 65) reasons.push(`Solid travel compatibility — you share enough preferences to enjoy trips together`);
    if (sections.travel < 50) concerns.push(`Different travel preferences could create tension on the road — these differences are worth discussing before booking anything`);
    if (ar.travel_style && br.travel_style && ar.travel_style === br.travel_style && ar.travel_style !== 'whatever') {
      reasons.push(`You share the same travel style (${ar.travel_style.replace('_', ' ')}) — no negotiating on accommodation standards`);
    }
    if (ar.travel_planning && br.travel_planning) {
      if (ar.travel_planning === 'full_itinerary' && br.travel_planning === 'spontaneous') {
        concerns.push(`Big planning mismatch: one of you craves full itineraries while the other prefers to wing it — you'll need a middle-ground system`);
      } else if (ar.travel_planning === 'spontaneous' && br.travel_planning === 'full_itinerary') {
        concerns.push(`Big planning mismatch: one of you craves full itineraries while the other prefers to wing it — you'll need a middle-ground system`);
      } else if (ar.travel_planning === br.travel_planning) {
        reasons.push(`Identical trip-planning philosophy — no one is packing itineraries while the other is booking flights two hours before departure`);
      }
    }
    if (ar.travel_pace && br.travel_pace) {
      if (ar.travel_pace === 'slow_deep' && br.travel_pace === 'fast_packed') {
        concerns.push(`Travel pace clash: one prefers slow, immersive exploration while the other wants to maximize every day — compromise is essential`);
      } else if (ar.travel_pace === 'fast_packed' && br.travel_pace === 'slow_deep') {
        concerns.push(`Travel pace clash: one prefers slow, immersive exploration while the other wants to maximize every day — compromise is essential`);
      } else if (ar.travel_pace === br.travel_pace) {
        reasons.push(`Same travel pace — no one will feel rushed or bored`);
      }
    }
    if (sections.lifestyle >= 70) reasons.push(`Strong lifestyle compatibility adds a solid foundation to your travel dynamic`);
    if (ar.travel_budget_split && br.travel_budget_split && ar.travel_budget_split === br.travel_budget_split) {
      reasons.push(`Aligned on how to split travel expenses — one less thing to argue about at the hotel check-in`);
    }
  }
...
function generateInsights(a, b, ar, br, sections) {
  ...
  const sectionLabels = {
    ...
    lifestyle: 'Lifestyle & Goals', communication: 'Communication', partnership: 'Partnership Philosophy',
    daily: 'Daily Habits', travel: 'Travel & Adventure',
  };
